export { estimadorSaludBateria } from './tool/batteryHealthEstimator/entry';
export type { EstimadorSaludBateriaLocaleContent } from './tool/batteryHealthEstimator/entry';
export { pixelesPantalla } from './tool/deadPixelTest/entry';
export type { PixelesPantallaLocaleContent } from './tool/deadPixelTest/entry';
export { testMando } from './tool/gamepadTest/entry';
export type { TestMandoLocaleContent } from './tool/gamepadTest/entry';
export { gamepadPollingRateChecker } from './tool/gamepadPollingRateChecker/entry';
export type { GamepadPollingRateCheckerLocaleContent } from './tool/gamepadPollingRateChecker/entry';
export { probadorVibracionMando } from './tool/gamepadVibrationTester/entry';
export type { ProbadorVibracionMandoLocaleContent } from './tool/gamepadVibrationTester/entry';
export { testTeclado } from './tool/keyboardTest/entry';
export type { TestTecladoLocaleContent } from './tool/keyboardTest/entry';
export { testRaton } from './tool/mousePollingTest/entry';
export type { TestRatonLocaleContent } from './tool/mousePollingTest/entry';
export { mouseDoubleClickTest } from './tool/mouseDoubleClickTest/entry';
export type { MouseDoubleClickTestLocaleContent } from './tool/mouseDoubleClickTest/entry';
export { mouseScrollTest } from './tool/mouseScrollTest/entry';
export type { MouseScrollTestLocaleContent } from './tool/mouseScrollTest/entry';
export { mouseDpiAnalyzer } from './tool/mouseDpiAnalyzer/entry';
export type { MouseDpiAnalyzerLocaleContent } from './tool/mouseDpiAnalyzer/entry';
export { toneGenerator } from './tool/toneGenerator/entry';
export type { ToneGeneratorLocaleContent } from './tool/toneGenerator/entry';
export { waterEjector } from './tool/waterEjector/entry';
export type { WaterEjectorLocaleContent } from './tool/waterEjector/entry';
export { subwooferCrossoverTest } from './tool/subwooferCrossoverTest/entry';
export type { SubwooferCrossoverTestLocaleContent } from './tool/subwooferCrossoverTest/entry';
export { speakerPhaseTester } from './tool/speakerPhaseTester/entry';
export type { SpeakerPhaseTesterLocaleContent } from './tool/speakerPhaseTester/entry';
export { refreshRateDetector } from './tool/refreshRateDetector/entry';
export type { RefreshRateDetectorLocaleContent } from './tool/refreshRateDetector/entry';
export { monitorGhostingTest } from './tool/monitorGhostingTest/entry';
export type { MonitorGhostingTestLocaleContent } from './tool/monitorGhostingTest/entry';
export { backlightBleedBloomingTest } from './tool/backlightBleedBloomingTest/entry';
export type { BacklightBleedBloomingTestLocaleContent } from './tool/backlightBleedBloomingTest/entry';
export { oledScreenBurnInFixer } from './tool/oledScreenBurnInFixer/entry';
export type { OledScreenBurnInFixerLocaleContent } from './tool/oledScreenBurnInFixer/entry';
export { spectrumCanvas } from './tool/colorAccuracyTest/entry';
export type { SpectrumCanvasLocaleContent } from './tool/colorAccuracyTest/entry';
export { upsRuntimeCalculator } from './tool/upsRuntimeCalculator/entry';
export type { UpsRuntimeCalculatorLocaleContent } from './tool/upsRuntimeCalculator/entry';
export { stereoAudioTest } from './tool/stereoAudioTest/entry';
export type { StereoAudioTestLocaleContent } from './tool/stereoAudioTest/entry';
export { webBluetoothBleScanner } from './tool/webBluetoothBleScanner/entry';
export type { WebBluetoothBleScannerLocaleContent } from './tool/webBluetoothBleScanner/entry';
export { webMidiKeyboardTester } from './tool/webMidiKeyboardTester/entry';
export type { WebMidiKeyboardTesterLocaleContent } from './tool/webMidiKeyboardTester/entry';
export { keyboardChatterTest } from './tool/keyboardChatterTest/entry';
export type { KeyboardChatterTestLocaleContent } from './tool/keyboardChatterTest/entry';
export { webUsbSerialMonitor } from './tool/webUsbSerialMonitor/entry';
export type { WebUsbSerialMonitorLocaleContent } from './tool/webUsbSerialMonitor/entry';
export { usbPowerBudgetCalculator } from './tool/usbPowerBudgetCalculator/entry';
export type { UsbPowerBudgetCalculatorLocaleContent } from './tool/usbPowerBudgetCalculator/entry';
export { mobileSensorTest } from './tool/mobileSensorTest/entry';
export type { MobileSensorTestLocaleContent } from './tool/mobileSensorTest/entry';
export { mouseJitterAngleSnappingTest } from './tool/mouseJitterAngleSnappingTest/entry';
export type { MouseJitterAngleSnappingTestLocaleContent } from './tool/mouseJitterAngleSnappingTest/entry';
export { touchScreenTester } from './tool/touchScreenTester/entry';
export type { TouchScreenTesterLocaleContent } from './tool/touchScreenTester/entry';
export { testInputLag } from './tool/inputLagTest/entry';
export type { InputLagTestLocaleContent } from './tool/inputLagTest/entry';
export { psuPowerRequirementCalculator } from './tool/psuPowerRequirementCalculator/entry';
export type { PsuPowerRequirementLocaleContent } from './tool/psuPowerRequirementCalculator/entry';
export { resistorColorCodeCalculator } from './tool/resistorColorCodeCalculator/entry';
export type { ResistorColorCodeLocaleContent } from './tool/resistorColorCodeCalculator/entry';
export { ledResistorCalculator } from './tool/ledResistorCalculator/entry';
export type { LedResistorLocaleContent } from './tool/ledResistorCalculator/entry';
export { phoneChargeTimeCalculator } from './tool/phoneChargeTimeCalculator/entry';
export type { PhoneChargeTimeCalculatorLocaleContent } from './tool/phoneChargeTimeCalculator/entry';
export { laptopBatteryRuntime } from './tool/laptopBatteryRuntimeCalculator/entry';
export type { LaptopBatteryRuntimeLocaleContent } from './tool/laptopBatteryRuntimeCalculator/entry';
import { audioDelayTest } from './tool/audioDelayTest/entry';
export { audioDelayTest } from './tool/audioDelayTest/entry';
export type { AudioDelayTestLocaleContent } from './tool/audioDelayTest/entry';
export { microphoneSpectrumTester } from './tool/microphoneSpectrumTester/entry';
export type { MicrophoneSpectrumTesterLocaleContent } from './tool/microphoneSpectrumTester/entry';
export { webcamTester } from './tool/webcamTester/entry';
export type { WebcamTesterLocaleContent } from './tool/webcamTester/entry';
export { hardwareCategory } from './category';
import { estimadorSaludBateria } from './tool/batteryHealthEstimator/entry';
import { pixelesPantalla } from './tool/deadPixelTest/entry';
import { testMando } from './tool/gamepadTest/entry';
import { gamepadPollingRateChecker } from './tool/gamepadPollingRateChecker/entry';
import { probadorVibracionMando } from './tool/gamepadVibrationTester/entry';
import { testTeclado } from './tool/keyboardTest/entry';
import { testRaton } from './tool/mousePollingTest/entry';
import { mouseDoubleClickTest } from './tool/mouseDoubleClickTest/entry';
import { mouseScrollTest } from './tool/mouseScrollTest/entry';
import { mouseDpiAnalyzer } from './tool/mouseDpiAnalyzer/entry';
import { toneGenerator } from './tool/toneGenerator/entry';
import { waterEjector } from './tool/waterEjector/entry';
import { subwooferCrossoverTest } from './tool/subwooferCrossoverTest/entry';
import { speakerPhaseTester } from './tool/speakerPhaseTester/entry';
import { refreshRateDetector } from './tool/refreshRateDetector/entry';
import { monitorGhostingTest } from './tool/monitorGhostingTest/entry';
import { backlightBleedBloomingTest } from './tool/backlightBleedBloomingTest/entry';
import { oledScreenBurnInFixer } from './tool/oledScreenBurnInFixer/entry';
import { spectrumCanvas } from './tool/colorAccuracyTest/entry';
import { upsRuntimeCalculator } from './tool/upsRuntimeCalculator/entry';
import { stereoAudioTest } from './tool/stereoAudioTest/entry';
import { webBluetoothBleScanner } from './tool/webBluetoothBleScanner/entry';
import { webMidiKeyboardTester } from './tool/webMidiKeyboardTester/entry';
import { keyboardChatterTest } from './tool/keyboardChatterTest/entry';
import { webUsbSerialMonitor } from './tool/webUsbSerialMonitor/entry';
import { usbPowerBudgetCalculator } from './tool/usbPowerBudgetCalculator/entry';
import { mobileSensorTest } from './tool/mobileSensorTest/entry';
import { mouseJitterAngleSnappingTest } from './tool/mouseJitterAngleSnappingTest/entry';
import { touchScreenTester } from './tool/touchScreenTester/entry';
import { testInputLag } from './tool/inputLagTest/entry';
import { psuPowerRequirementCalculator } from './tool/psuPowerRequirementCalculator/entry';
import { resistorColorCodeCalculator } from './tool/resistorColorCodeCalculator/entry';
import { ledResistorCalculator } from './tool/ledResistorCalculator/entry';
import { microphoneSpectrumTester } from './tool/microphoneSpectrumTester/entry';
import { webcamTester } from './tool/webcamTester/entry';
import { voltageDividerCalculator } from './tool/voltageDividerCalculator/entry';
import { ohmLawPowerCalculator } from './tool/ohmLawPowerCalculator/entry';
import { pcbTraceWidthImpedanceChecker } from './tool/pcbTraceWidthImpedanceChecker/entry';
import { phoneChargeTimeCalculator } from './tool/phoneChargeTimeCalculator/entry';
import { laptopBatteryRuntime } from './tool/laptopBatteryRuntimeCalculator/entry';
export type { OhmLawPowerCalculatorLocaleContent } from './tool/ohmLawPowerCalculator/entry';
export { ohmLawPowerCalculator } from './tool/ohmLawPowerCalculator/entry';
export type { PcbTraceWidthImpedanceCheckerLocaleContent } from './tool/pcbTraceWidthImpedanceChecker/entry';
export { pcbTraceWidthImpedanceChecker } from './tool/pcbTraceWidthImpedanceChecker/entry';
export const ALL_ENTRIES = [estimadorSaludBateria, pixelesPantalla, touchScreenTester, testMando, gamepadPollingRateChecker, probadorVibracionMando, testTeclado, keyboardChatterTest, webMidiKeyboardTester, testRaton, mouseDoubleClickTest, mouseScrollTest, mouseDpiAnalyzer, mouseJitterAngleSnappingTest, toneGenerator, waterEjector, subwooferCrossoverTest, speakerPhaseTester, microphoneSpectrumTester, webcamTester, refreshRateDetector, monitorGhostingTest, backlightBleedBloomingTest, oledScreenBurnInFixer, spectrumCanvas, upsRuntimeCalculator, usbPowerBudgetCalculator, mobileSensorTest, stereoAudioTest, webBluetoothBleScanner, webUsbSerialMonitor, testInputLag, psuPowerRequirementCalculator, resistorColorCodeCalculator, ledResistorCalculator, audioDelayTest, voltageDividerCalculator, ohmLawPowerCalculator, pcbTraceWidthImpedanceChecker, phoneChargeTimeCalculator, laptopBatteryRuntime];
