...
export interface HardwareToolEntry<TUI extends Record<string, string> = Record<string, string>> {
  id: string;
  icons: {
    bg: string;
    fg: string;
...
export interface HardwareCategoryEntry {
  icon: string;
  tools: HardwareToolEntry[];
  i18n: LocaleMap<CategoryLocaleContent>;
}

export interface ToolDefinition {
  entry: HardwareToolEntry;
  Component: unknown;
