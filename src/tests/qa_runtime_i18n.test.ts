...
function isTechnicalLiteral(value: string): boolean {
  return ['.', '#', '['].some((prefix) => value.startsWith(prefix))
    ...
    || /^(?:axis|Gamepad)$/.test(value)
    || /^\$\{[^}]+\}(?:V|°C|s|ms| Hz)$/.test(value);
}

function shouldSkipLiteral(value: string | undefined, beforeMatch: string): boolean {
  if (value === undefined) return true;
  if (/(?:===|!==|==|!=)\s*$/.test(beforeMatch)) return true;
  return isTechnicalLiteral(value);
}

function visibleLiteralsInLine(line: string, index: number): VisibleLiteral[] {
  const failures: VisibleLiteral[] = [];
  if (!domTextAssignment.test(line)) return failures;

  const assignment = line.slice(line.search(domTextAssignment));
  for (const match of assignment.matchAll(quotedLiteral)) {
    const value = match[2];
    const beforeMatch = assignment.slice(0, match.index ?? 0);
    if (shouldSkipLiteral(value, beforeMatch)) continue;
...
function hardcodedVisibleLiterals(source: string): VisibleLiteral[] {
  return source.split(/\r?\n/).flatMap((line, index) => visibleLiteralsInLine(line, index));
}
