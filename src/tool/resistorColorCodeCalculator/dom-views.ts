...
export function renderResistorScene(colors: ResistorColor[], result: ResistorResult, ui: ResistorColorCodeUI): string {
  ...
  const resultText = result.valid ? result.valueOhms + ' Ω' : ui.statusInvalid;
  return '<div class="resistor-scene" aria-label="' + resultText + '"><svg class="resistor-svg" data-resistor-svg viewBox="0 0 680 220" role="img" aria-label="' + resultText + '"><line class="resistor-wire-svg" x1="24" y1="110" x2="182" y2="110"></line><rect class="resistor-body-svg" x="160" y="44" width="352" height="132" rx="28"></rect>' + bands + '<line class="resistor-wire-svg" x1="512" y1="110" x2="656" y2="110"></line></svg><div class="resistor-scene-caption"><span>' + ui.resistance + '</span><strong>' + resultText + '</strong></div></div>';
}
